...
clean_data = tf.data.Dataset.from_tensors(
    tf.random.normal(shape=(8, 256, 256, 3), dtype=tf.float32)
)
dirty_data = tf.data.Dataset.from_tensors(
    tf.random.uniform(shape=(8, 256, 256, 3), dtype=tf.float32)
)
