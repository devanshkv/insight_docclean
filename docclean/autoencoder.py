...
class Autoencoder:
    ...
    def build_model(self):
        ...
        e = tf.keras.layers.Conv2D(32, (4, 4), strides=2, activation="relu")(
            input_layer
        )
        ...
        output_layer = tf.keras.layers.Conv2D(
            self.channels, (3, 3), activation="sigmoid", padding="same"
        )(d)
    ...
    def train_model(
        ...
        if validation_data is None:
            self.history = self.autoencoder_model.fit(
                train_dataset, epochs=epochs, callbacks=[early_stopping], verbose=1
            )
        else:
            self.history = self.autoencoder_model.fit(
                train_dataset,
